...
<div id="layoutSidenav_content">
    <div class="container p-4">
        <h2 class="mb-4 text-utama text-uppercase fw-bolder text-xl">Kategori</h2>
        @if(session('success'))
            <div id="alert" class="alert alert-success">{{ session('success') }}</div>
        @elseif(session('error'))
            <div id="alert" class="alert alert-danger">{{ session('error') }}</div>
        @endif
        <a href="/produks/kategori/create" class="btn btn-success mb-4">Tambah Kategori</a>
                ...
                <table class="table table-striped table-bordered order-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>Kategori {{ $item->nama }}</td>
                            <td>
                                <a href="{{ route('admin.kategori.edit', $item->id) }}" class="btn btn-primary btn-sm">Edit</a>
